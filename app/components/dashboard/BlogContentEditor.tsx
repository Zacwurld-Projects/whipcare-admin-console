...
function ToolbarSelect({
  ...
  return (
    <label className={`inline-flex shrink-0 items-center gap-1 ${className}`}>
      {showLabel ? (
      ...
      <span className="relative inline-flex items-center">
        <select
          ...
          className={`h-8 appearance-none rounded-md border border-[#E5E7EB] bg-white py-1 pl-2 pr-6 text-xs text-[#1E2939] outline-none focus:border-[#FE915D] ${
            compact ? "min-w-[6.25rem] max-w-[9rem]" : "min-w-[7.5rem] max-w-[11rem]"
          }`}
        >
          {children}
        </select>
...
function EditorToolbar({
  ...
  return (
    <div
      ...
      className="flex flex-wrap items-center gap-1 overflow-x-auto overscroll-x-contain px-3 py-2 touch-manipulation"
    >
      <ToolbarSelect
        label="Font"
        showLabel
      ...
      <ToolbarButton label="Insert link" active={editor.isActive("link")} onClick={onInsertLink}>
        <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
          <path
