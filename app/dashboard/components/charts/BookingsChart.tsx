...
import { fetchServiceBookingsSummary } from '@/app/api/apiClient';
...
const BookingsChart = ({
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        setLoading(true);
        const response = await fetchServiceBookingsSummary();
        const allMonths = Array(12).fill(0);

        response?.data?.forEach((entry: { month: number; numberOfBookings: number }) => {
          const monthIndex = entry.month - 1;
          if (monthIndex >= 0 && monthIndex < 12) {
            allMonths[monthIndex] = entry.numberOfBookings;
          }
        });
  ...
  return (
    ...
    >
      <div className='mb-[30px] flex items-end justify-between'>
        <h6 className='heading-h6 p-2 font-semibold dark:text-white'>{heading || 'Bookings'}</h6>
