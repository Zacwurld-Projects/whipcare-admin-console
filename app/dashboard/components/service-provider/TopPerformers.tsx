        ...
        <ul className='flex w-max min-w-full justify-between gap-8'>
          {topPerformers.map((item, index) => (
            <li key={index} className='flex-column items-center gap-2'>
              ...
              <div>
                <p className='text-xsmall font-semibold text-gray-800 dark:text-white'>
                  {index + 1}. {item.firstName} {item.lastName}
                </p>
                <p className='text-xsmall text-gray dark:text-dark-tertiary'>{item.serviceType}</p>
              </div>
              <p className='rounded-[18px] bg-[#ff915b] px-4 py-[5px] text-[8px] text-white dark:bg-dark-accent'>
